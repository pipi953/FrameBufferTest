...
int main()
{
	...
	//	加载纹理
	unsigned int cubeTexture = loadTexture("Resources/Textures/test.jpg");
	unsigned int floorTexture = loadTexture("Resources/Textures/2.jpg");
	...
	//	创建颜色附件（颜色挂载）
	unsigned int textureColorbuffer;		//	声明颜色挂载
	glGenTextures(1, &textureColorbuffer);		//	创建颜色挂载
	glBindTexture(GL_TEXTURE_2D, textureColorbuffer);	//	绑定颜色挂载，即：告诉程序，我们现在要对创建的颜色挂载进行操作，保证此操作不影响其他颜色挂载
	...
	//为深度和模板附件创建一个renderbuffer对象（我们不会对它们进行采样）
	//	方法1
	//	创建RBO，包含深度附件 与 模板附件
	unsigned int rbo;		//	声明
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//	----------------------开始渲染场景---------------------------------
		 //	绑定到帧缓冲区并绘制场景，就像我们平时绑定着色纹理一样
		//	切换到自定义的帧缓冲区
		glBindFramebuffer(GL_FRAMEBUFFER, fbo);
		glEnable(GL_DEPTH_TEST); // 启用深度测试（禁用渲染屏幕空间四边形）
		...
		//	摄像机/视图 转换
		float angle = -25.0f;
		glm::mat4 model = glm::rotate(model, glm::radians(angle), glm::vec3(0.0f, 0.3f, 0.0f));
		...
		//	现在绑定回默认的帧缓冲区，并绘制带有附加帧缓冲颜色纹理的四边形平面
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		glDisable(GL_DEPTH_TEST); //	禁用深度测试，因此深度测试不会丢弃屏幕空间四元组。
		//	清除所有相关缓冲区
		glClearColor(1.0f, 1.0f, 1.0f, 1.0f); // 设置清除的颜色为白色（实际上并不是真的，因为我们是不能看到四边形的）
